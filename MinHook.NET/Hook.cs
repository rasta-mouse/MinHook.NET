...
internal sealed class Hook
{
    ...
    public void Enable(bool enable = true)
    {
        ...
        var patchTarget = trampoline.Target;
        ...
        if (enable)
        {
            ...
            var jmp = new JMPCALL_REL(0xE9);

            if (IntPtr.Size == 8)
            {
                var rel = (long)trampoline.Relay - ((long)patchTarget + Marshal.SizeOf(jmp));
            ...
            if (trampoline.PatchAbove)
            {
                var shortJmp = new JMP_REL_SHORT(0xEB,
                    (byte)(0 - (Marshal.SizeOf(typeof(JMP_REL_SHORT)) + Marshal.SizeOf(jmp))));
        ...
        else
        {
            ...
            if (trampoline.PatchAbove)
            {
                Marshal.Copy(trampoline.Backup, 0, patchTarget,
                    Marshal.SizeOf(typeof(JMPCALL_REL)) + Marshal.SizeOf(typeof(JMP_REL_SHORT)));
            }
            else
            {
