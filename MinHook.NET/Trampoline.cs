...
internal sealed class Trampoline
{
    ...
    [SuppressMessage("CodeQuality", "IDE0052:Remove unread private members",
        Justification = "MemorySlot has unmanged resources that we need to hold onto to prevent GC")]
    ...
    public Trampoline(IntPtr target, IntPtr detour, MemorySlot memorySlot)
    {
        ...
        uint oldPos = 0;
        uint newPos = 0;
        var jmpDest = IntPtr.Zero;
        var instBuf = Marshal.AllocHGlobal(16);
        var finished = false;
        ...
        do
        {
            ...
            uint copySize;
            IntPtr copySrc;
            var oldInst = (IntPtr)((long)Target + oldPos);
            var newInst = (IntPtr)((long)Tramp + newPos);

            var disasm = new Disassembler(oldInst, 16, is64 ? ArchitectureMode.x86_64 : ArchitectureMode.x86_32, 0,
                true);
            disasm.Disassemble();

            var inst = disasm.NextInstruction();
            ...
            else if (is64 && inst.HasModRM && (inst.ModRM & 0xC7) == 0x5)
            {
                ...
                var modRMReg = (byte)((inst.ModRM & 0x3f) >> 3);
                copySrc = instBuf;
                CopyMemory(instBuf, oldInst, (int)copySize);

                //TODO: figure out how to calculate imm length with udisasm engine
                uint immLen = 0;
                var relAddr = (IntPtr)((long)instBuf + inst.Length - immLen - 4);
                var updatedRel = (uint)((ulong)oldInst + (ulong)inst.Length + inst.Operands[0].LvalUDWord -
                                        ((ulong)newInst + (ulong)inst.Length));
            ...
            else if (inst.Mnemonic == ud_mnemonic_code.UD_Icall)
            {
                ...
                var dest = (IntPtr)((long)oldInst + inst.Length + inst.Operands[0].LvalSDWord);
            ...
            else if ((inst.Bytes[0] & 0xFD) == 0xE9)
            {
                ...
                var dest = oldInst + inst.Length;

                if (inst.Bytes[0] == 0xE9)
                {
                    //short JMP
                     ...
                     (inst.Bytes[0] == 0xF0 && (inst.Bytes[1] & 0xF0) == 0x80))
            {
                ...
                else
                {
                    ...
                    var cond = (byte)((inst.Bytes[0] != 0x0F ? inst.Bytes[0] : inst.Bytes[1]) & 0xF0);
            ...
            if (srcObj != null)
            {
                var data = Marshal.AllocHGlobal((int)copySize);
            ...
            && IsCodePadding((IntPtr)((long)Target + oldPos), (int)(Marshal.SizeOf(typeof(JMPCALL_REL)) - oldPos)))
        {
            ...
            if (oldPos < Marshal.SizeOf(typeof(JMP_REL_SHORT))
                && IsCodePadding((IntPtr)((long)Target + oldPos),
                    (int)(Marshal.SizeOf(typeof(JMP_REL_SHORT)) - oldPos)))
            {
            ...
            if (!IsCodePadding((IntPtr)((long)Target - Marshal.SizeOf(typeof(JMPCALL_REL))),
                    Marshal.SizeOf(typeof(JMPCALL_REL))))
            {
                throw new Exception("Not enought space for patch");
            }
        ...
        if (is64)
        {
            ...
            var data = Marshal.AllocHGlobal((int)Marshal.SizeOf(jmp));
    ...
    bool IsCodePadding(IntPtr codePtr, int len)
    {
        ...
        for (var idx = 1; idx < code.Length; ++idx)
        {
            if (code[idx] != code[0])
                return false;
        }
