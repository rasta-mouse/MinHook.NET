...
public sealed class HookEngine : IDisposable
{
    ...
    MemoryAllocator memoryAllocator = new();
    Dictionary<Delegate, Hook> originalHookMapping = new();
    Dictionary<Delegate, Hook> detourHookMapping = new();
    List<IntPtr> suspendedThreads = new();

    public Func CreateHook<Func>(string dll, string function, Func detour) where Func : Delegate
    {

        var target = GetProcAddress(GetModuleHandle(dll), function);
    ...
    public Func CreateHook<Func>(IntPtr target, Func detour) where Func : Delegate
    {
        ...
        lock (this)
        {
            var hook = new Hook(target, Marshal.GetFunctionPointerForDelegate(detour),
                memoryAllocator.AllocateBuffer(target));
            var original = (Func)Marshal.GetDelegateForFunctionPointer(hook.Original, typeof(Func));
